...
- Use the `--no-save` flag to install a package into `node_modules/` and update `kley.lock` **without modifying `package.json`**. Useful for temporary testing of a local package without creating noise in version control. Note: Yarn v1 does not support this flag natively — `package.json` will still be modified when using Yarn v1.
