...
### Changed
- **`install`/`add` commands**: Now strip `devDependencies` from a library's `package.json` when it is copied into a project's `.kley/` directory. This prevents the consumer from installing the library's development dependencies, reducing `node_modules` bloat and speeding up installation.
