# kley 🩹

[![Release](https://github.com/sumbad/kley/actions/workflows/release.yml/badge.svg)](https://github.com/sumbad/kley/releases)

**Простой локальный менеджер пакетов для npm (JS/TS)**

> Как **npm link**, но более удобный. Как **yalс** но без зависимости от Node.js

[English](./README.md) | Русский

**kley** — это утилита командной строки, которая упрощает локальную разработку npm-пакетов. Она предоставляет надежную альтернативу `npm link`, управляя локальным хранилищем пакетов. Вы можете "публиковать" пакеты в центральный кэш на вашем компьютере и "добавлять" их в свои проекты путем прямого копирования файлов, что позволяет избежать распространенных проблем с символическими ссылками.

## Ключевые особенности

- **Независим от Node.js**: Можно публиковать и устанавливать пакет даже если библиотека и хост-проект имеют разные версии Node.js.
- **Быстрый, экономичный и безопасный**: Написан на Rust с гарантией от утечек памяти, для максимальной безопасности и производительности.
- **Надежный**: Избегает проблем с символическими ссылками, копируя файлы напрямую.
- **Простой API**: Четыре основные команды для работы: `publish`, `add`, `link`, и `remove`.
- **Кроссплатформенный**: Работает на macOS, Linux и Windows.

## Установка

### Быстрая установка (рекомендуется)

Вы можете установить `kley` одной командой с помощью установочного скрипта:

```bash
# Linux / macOS
curl --proto '=https' --tlsv1.2 -LsSf https://github.com/sumbad/kley/releases/latest/download/kley-installer.sh | sh
```
```bash
# Windows
powershell -ExecutionPolicy Bypass -c "irm https://github.com/sumbad/kley/releases/latest/download/kley-installer.ps1 | iex"
```

### Ручная установка

В качестве альтернативы, вы можете установить `kley`, скачав готовый бинарный файл со страницы [**Релизов**](https://github.com/sumbad/kley/releases).

1.  Скачайте архив, подходящий для вашей системы (например, `kley-x86_64-apple-darwin.tar.gz`).
2.  Распакуйте архив.
3.  Переместите бинарный файл `kley` в директорию, которая находится в `PATH` вашей системы (например, `/usr/local/bin` для macOS/Linux).


### Установка через Cargo (crates.io)
Если у вас установлен Rust и Cargo, вы можете установить `kley` напрямую с crates.io:

```bash
cargo install kley
```

### Установка через npm (kley-cli)
Вы можете установить `kley-cli` глобально. Используйте этот способ, только если у вас одинаковые версии Node.js для вашей библиотеки и хост-проекта, в противном случае вам придётся устанавливать `kley-cli` для всех версий Node.js:

```bash
npm install -g kley-cli
```

## Использование

### 1. `kley publish`
Выполните эту команду в директории пакета, которым вы хотите поделиться локально. Kley скопирует все необходимые файлы в центральное хранилище по пути `~/.kley/packages/<имя-вашего-пакета>`.

### 2. `kley add <имя-пакета>`
Выполните эту команду в проекте, где вы хотите использовать ваш локальный пакет. Kley скопирует пакет в локальную директорию `./.kley/`, а затем автоматически обновит ваши `package.json` и `kley.lock`.

- Используйте флаг `--dev`, чтобы добавить пакет в `devDependencies`.

### 3. `kley link <имя-пакета>`
Добавление пакета без изменения `package.json` файла через линк в `node_modules` на его локальную версию. Сначала копирует пакет в локальную директорию `.kley`, а затем создает символическую ссылку из `./.kley/<имя-вашего-пакета>` в директорию `node_modules` вашего проекта. Это быстрее, чем полная команда `add`, и **не изменяет** ваш `package.json`.

### 4. `kley remove <имя-пакета>`
Выполните эту команду, чтобы чисто удалить управляемую kley зависимость из вашего проекта. Она обновит `package.json` и `kley.lock`, а также удалит файлы пакета из директории `./.kley/`.

- Используйте флаг `--all`, чтобы удалить все управляемые kley пакеты из проекта.

## Участие в разработке

Мы приветствуем любой вклад! Пожалуйста, не стесняйтесь отправлять Pull Request.

## О проекте

Этот проект вдохновлен таким замечательным инструментом, как [yalc](https://github.com/wclr/yalc). Главное преимущество `kley` в том, что это единый, самодостаточный бинарный файл, **не имеющий зависимости от Node.js**. Это означает, что вы можете управлять пакетами независимо от вашей текущей версии Node.js или каких-либо проблем с самим `npm`.

> **Примечание:** Этот проект находится в активной разработке и в настоящее время поддерживает только базовые команды. Больше функций появится в ближайшее время!

## Лицензия

Проект лицензирован под лицензией MIT - подробности см. в файле LICENSE.
