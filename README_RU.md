...
- Используйте флаг `--no-save`, чтобы установить пакет в `node_modules/` и обновить `kley.lock` **без изменения `package.json`**. Удобно для временного тестирования локального пакета без лишних изменений в системе контроля версий. Примечание: Yarn v1 не поддерживает этот флаг нативно — при использовании Yarn v1 `package.json` всё равно будет изменён.
