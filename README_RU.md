...
### 8. `kley watch [путь]`
Запустите эту команду в директории вашего локального пакета для отслеживания изменений в файлах. Она автоматически выполняет `kley publish --push` при каждом обнаружении изменений, обеспечивая непрерывный рабочий процесс (hot-reloading), при котором использующие пакет проекты мгновенно получают обновления.

- **С указанием пути**: `kley watch <путь>` (например, `kley watch src`) отслеживает изменения только внутри указанной поддиректории. Это крайне рекомендуется для крупных проектов, чтобы снизить нагрузку на системный наблюдатель файловой системы.
