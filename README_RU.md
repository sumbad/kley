...
- **Без имени пакета**: `kley install` устанавливает все пакеты, перечисленные в `kley.lock`. Это удобно для восстановления всех локальных зависимостей, аналогично тому, как `npm install` обновляет `node_modules` на основе `package.json`.
